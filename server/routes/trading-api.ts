...
router.post('/command', async (req, res) => {
  try {
    ...
    await databaseUUIDFixer.logWalletActivitySafely({
      walletAddress: 'system',
      activityType: 'command_execution',
      metadata: { command, params, result: result.message }
    });
...
router.post('/communicate', async (req, res) => {
  try {
    ...
    await databaseUUIDFixer.logWalletActivitySafely({
      walletAddress: 'system',
      activityType: 'ai_communication',
      metadata: { userMessage: message, aiResponse: response }
    });
...
router.post('/auth-event', async (req, res) => {
  try {
    ...
    await databaseUUIDFixer.logWalletActivitySafely({
      walletAddress: data.wallet || 'system',
      activityType: 'auth_event',
      metadata: { event, data, timestamp }
    });
